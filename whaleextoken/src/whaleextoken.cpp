...
namespace eosio {
	...
	void token::close(name owner, symbol_type symbol) {
		require_auth(owner);
	...
	void token::addblacklist(name account, symbol_type symbol) {
		auto sym_name = symbol.name();
		stats statstable(_self, sym_name);
		auto existing = statstable.find(sym_name);
		eosio_assert(existing != statstable.end(),
				"token with symbol does not exist, create token before add to black list");
		const auto& st = *existing;
	...
	void token::rmblacklist(name account, symbol_type symbol) {
		auto sym_name = symbol.name();
		stats statstable(_self, sym_name);
		auto existing = statstable.find(sym_name);
		eosio_assert(existing != statstable.end(),
				"token with symbol does not exist, create token before remove from black list");
		const auto& st = *existing;
		...
		eosio_assert(it != blacklists.end(),
				"Blacklist row already deleted or never existed. Action won't have any effect.");
		blacklists.erase(it);
	}

} /// namespace eosio

EOSIO_DISPATCH( eosio::token, (create)(issue)(transfer)(close)(retire)(addblacklist)(rmblacklist) )
